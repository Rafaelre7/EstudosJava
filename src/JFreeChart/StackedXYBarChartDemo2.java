...
public class StackedXYBarChartDemo2 extends ApplicationFrame {
	...
	private static JFreeChart createChart(TableXYDataset dataset) {
		...
		renderer.setBaseItemLabelsVisible(true); //COLOCA A LEGENDA ENCIMA  DE CADA COLUNA
		renderer.setBaseItemLabelGenerator(new StandardXYItemLabelGenerator());
		renderer.setBasePositiveItemLabelPosition(
				new ItemLabelPosition(ItemLabelAnchor.OUTSIDE12, TextAnchor.BOTTOM_CENTER));

		renderer.setBaseToolTipGenerator(new StandardXYToolTipGenerator("{0} : {1} = {2}", new SimpleDateFormat("yyyy"),
				new DecimalFormat("0")));
